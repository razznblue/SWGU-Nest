import { Toon } from '../toons/toons.model';

/**
 * A class representing a Toon going into battle or doing another action.
 * This class becomes a copy of @alias Toon
 * This is helpful for battles. After the battle is finished, the TempToon is deleted, and the Toon is updated.
 */
export class TempToon {
  name: string;
  tags: string[];

  /**
   * Assign values to a TempToon at creation time.
   * @param toonData An object containing toon data
   */
  constructor(toonData: Toon) {
    for (const key in toonData) {
      this[key] = toonData[key];
      console.log(`Created new TempToon: ${this.name}`);
    }
  }
